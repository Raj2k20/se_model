...
public class TrackDao extends BaseDao<TrackDto, TrackCriteria> {
    ...
    protected QueryParam getQueryParam(TrackCriteria criteria, FilterCriteria filterCriteria) {
        ...
        if (criteria.getUserId() != null) {
            criteriaList.add("(t.user_id=:userId or t.hidden = 0)");
            parameterMap.put("userId", criteria.getUserId());
        }
    ...
    public String create(Track track) {
        ...
        handle.createStatement("insert into " +
                "  t_track(id, album_id, artist_id, filename, title, titlecorrected, year, genre, length, bitrate, number, vbr, format, createdate, userid, hidden)" +
                "  values(:id, :albumId, :artistId, :fileName, :title, :titleCorrected, :year, :genre, :length, :bitrate, :number, :vbr, :format, :createDate, :userId, :hidden)")
                .bind("id", track.getId())
